...
RSpec.describe "/api/game_systems/armies", type: :request do
  let!(:user) { create(:user) }
  let!(:wargame) { create(:wargame) }
  let!(:armies) { create_list(:army, 2, game_system: wargame) }
  ...
  describe "with jwt login" do
    describe "index" do
      ...
      before do
        get_with_token(user, api_game_system_armies_path(wargame))
      end
